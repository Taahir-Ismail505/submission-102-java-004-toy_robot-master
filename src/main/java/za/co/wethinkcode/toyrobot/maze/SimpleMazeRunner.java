...
public class SimpleMazeRunner implements MazeRunner {
    ...
    private int runCost;
    /**
     * SimpleMazeRunner constructor
     */
    public SimpleMazeRunner(Robot target) {
        this.robot = target;
        this.world = this.robot.getWorld();
        this.maze = ((AbstractWorld) this.world).getMaze();
    }
    /**
    ...
    public boolean nextBlocked(int directionModifier) {
        ...
        if (directionToCheck == 0) {
            return maze.blocksPath(world.getPosition(), new Position(world.getPosition().getX(), world.getPosition().getY() + 1));
        } else if (directionToCheck == 1) {
            return maze.blocksPath(world.getPosition(), new Position(world.getPosition().getX() + 1, world.getPosition().getY()));
